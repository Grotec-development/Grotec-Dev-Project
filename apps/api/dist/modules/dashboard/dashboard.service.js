...
exports.DashboardService = exports.PIPELINE_STATES = void 0;
exports.toDashboardRange = toDashboardRange;
exports.toPipelineState = toPipelineState;
...
let DashboardService = class DashboardService {
    ...
    async summary(actor, range = 'day') {
        ...
        return {
            ...
            pipeline: exports.PIPELINE_STATES.map((state) => ({ state, count: segmented.counts[state] ?? 0 })),
    ...
    async pipeline(actor, state) {
        if (!exports.PIPELINE_STATES.includes(state)) {
            state = 'never_reached';
        }
    ...
    async segmentPipeline(actor) {
        ...
        for (const row of rows) {
            const state = exports.PIPELINE_STATES.includes(row.state) ? row.state : 'never_reached';
            byState[state].push(row.id);
        }
        const counts = { total: rows.length };
        for (const state of exports.PIPELINE_STATES)
...
function toDashboardRange(value) {
    if (value === 'week' || value === 'month')
        return value;
    return 'day';
}
function toPipelineState(value) {
    if (value && exports.PIPELINE_STATES.includes(value))
        return value;
