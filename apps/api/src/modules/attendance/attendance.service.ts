...
export class AttendanceService {
  ...
  async list(
    ...
    if (actor.roleCode === 'FOUNDER') {
        ...
        if (filters.employeeId !== actor.id) {
          ...
          if (!target || !outranks(actor.roleCode as RoleCode, target.role.code as RoleCode)) {
            throw ApiError.forbidden(
              ...
              'You can only view attendance for roles strictly below your rank (PRD §5.1.2)',
            );
          }
        }
      ...
      if (filters.employeeId && filters.employeeId !== actor.id) {
        ...
        if (!assigned) {
          throw ApiError.forbidden(
            'ROLE_HIERARCHY_FORBIDDEN',
            'You can only view attendance for yourself or assigned employees',
          );
        }
        conditions.push({ employeeId: filters.employeeId });
      } else {
      ...
      if (filters.employeeId && filters.employeeId !== actor.id) {
        throw ApiError.forbidden(
          'ROLE_HIERARCHY_FORBIDDEN',
          'You can only view your own attendance records (PRD §5.1.2)',
        );
      }
    ...
    const where: Prisma.AttendanceRecordWhereInput = conditions.length > 0 ? { AND: conditions } : {};
  ...
  async getSummary(actor: AuthEmployee, query: { month: string; employeeId?: string }) {
    ...
    if (actor.roleCode === 'FOUNDER') {
      ...
      if (query.employeeId) {
        if (query.employeeId !== actor.id) {
          ...
          if (!target || !outranks(actor.roleCode as RoleCode, target.role.code as RoleCode)) {
            throw ApiError.forbidden(
              'ROLE_HIERARCHY_FORBIDDEN',
              'You can only view attendance summary for roles strictly below your rank (PRD §5.1.2)',
            );
          }
        }
      ...
      if (query.employeeId && query.employeeId !== actor.id) {
        throw ApiError.forbidden(
          'ROLE_HIERARCHY_FORBIDDEN',
          'You can only view your own attendance summary (PRD §5.1.2)',
        );
      }
      conditions.push({ employeeId: actor.id });
    }
  ...
  async mark(actor: AuthEmployee, dto: MarkAttendanceDto) {
    ...
    if (!isSelf && actor.roleCode !== 'FOUNDER') {
      if (!outranks(actor.roleCode as RoleCode, employee.role.code as RoleCode)) {
        throw ApiError.forbidden(
          'ROLE_HIERARCHY_FORBIDDEN',
          'You can only mark attendance for roles strictly below your rank (PRD §5.1.2)',
        );
      }
    }
    ...
    const initialApprovalStatus: ApprovalStatus = canApprove
  ...
  async bulkMark(actor: AuthEmployee, dto: BulkAttendanceDto) {
    if (actor.roleCode === 'STAFF' || actor.roleCode === 'AGENT' || actor.roleCode === 'DELIVERY') {
      throw ApiError.forbidden(
        'ROLE_HIERARCHY_FORBIDDEN',
        'Staff and agents cannot perform bulk attendance operations',
      );
    }
    ...
    if (actor.roleCode !== 'FOUNDER') {
      ...
      for (const emp of employees) {
        if (!outranks(actor.roleCode as RoleCode, emp.role.code as RoleCode)) {
          throw ApiError.forbidden(
            'ROLE_HIERARCHY_FORBIDDEN',
            `Cannot mark attendance for employee ${emp.fullName} at or above your rank (PRD §5.1.2)`,
          );
        }
      }
    }
    ...
    let skippedCount = 0;
    let failedCount = 0;
    const results: Array<{ employeeId: string; status: string; error?: string }> = [];
  ...
  async correct(actor: AuthEmployee, id: string, dto: CorrectAttendanceDto) {
    ...
    if (!existing) throw ApiError.notFound('ATTENDANCE_NOT_FOUND', 'Attendance record not found');

    if (actor.roleCode !== 'FOUNDER') {
      if (!outranks(actor.roleCode as RoleCode, existing.employee.role.code as RoleCode)) {
        throw ApiError.forbidden(
          'ROLE_HIERARCHY_FORBIDDEN',
          'You can only correct attendance for roles strictly below your rank (PRD §5.1.2)',
        );
      }
    }
  ...
  async approve(actor: AuthEmployee, id: string) {
    const existing = await this.prisma.attendanceRecord.findUnique({
      where: { id },
      include: { employee: { include: { role: true } } },
    });
    ...
    if (actor.roleCode !== 'FOUNDER') {
      if (!outranks(actor.roleCode as RoleCode, existing.employee.role.code as RoleCode)) {
        throw ApiError.forbidden(
          'ROLE_HIERARCHY_FORBIDDEN',
          'You can only approve attendance for roles strictly below your rank (PRD §5.1.2)',
        );
      }
    }
  ...
  async reject(actor: AuthEmployee, id: string, dto: RejectAttendanceDto) {
    ...
    if (actor.roleCode !== 'FOUNDER') {
      if (!outranks(actor.roleCode as RoleCode, existing.employee.role.code as RoleCode)) {
        throw ApiError.forbidden(
          'ROLE_HIERARCHY_FORBIDDEN',
          'You can only reject attendance for roles strictly below your rank (PRD §5.1.2)',
        );
      }
    }
  ...
  async handleEsslWebhook(secret: string | undefined, dto: EsslWebhookDto) {
  ...
  async syncEssl(actor: AuthEmployee, dto: SyncEsslDto) {
  ...
  async createDevice(dto: CreateEsslDeviceDto) {
    const existing = await this.prisma.esslDevice.findUnique({ where: { deviceCode: dto.deviceCode } });
    if (existing) throw ApiError.conflict('DEVICE_EXISTS', 'Device code already registered');
    return this.prisma.esslDevice.create({ data: dto });
  }

  async getMappings(deviceId?: string) {
  ...
  async createMapping(dto: CreateEsslMappingDto) {
    ...
    if (existing) throw ApiError.conflict('MAPPING_EXISTS', 'Biometric PIN already mapped on this device');
