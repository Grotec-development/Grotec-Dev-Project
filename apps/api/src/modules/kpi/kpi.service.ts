...
export class KpiService {
  ...
  async getTargets(actor: AuthEmployee, query: { period?: string; employeeId?: string; roleCode?: string; teamId?: string }) {
    const conditions: Prisma.KpiTargetWhereInput[] = [];
    if (query.period) conditions.push({ period: query.period });
    if (query.teamId) conditions.push({ OR: [{ teamId: query.teamId }, { team: query.teamId }] });

    if (actor.roleCode === 'FOUNDER') {
      if (query.employeeId) conditions.push({ employeeId: query.employeeId });
      if (query.roleCode) conditions.push({ roleCode: query.roleCode });
        ...
        if (query.employeeId !== actor.id) {
          ...
          if (!target || !outranks(actor.roleCode as RoleCode, target.role.code as RoleCode)) {
            throw ApiError.forbidden(
              ...
              'You can only view KPI targets for roles strictly below your rank (PRD §5.1.2)',
            );
          }
        }
      ...
      if (query.employeeId && query.employeeId !== actor.id) {
        throw ApiError.forbidden(
          'ROLE_HIERARCHY_FORBIDDEN',
          'You can only view your own KPI targets (PRD §5.1.2)',
        );
      }
  ...
  async upsertTarget(actor: AuthEmployee, dto: UpsertKpiTargetDto) {
    ...
    if ((hasEmployee && hasTeam) || (!hasEmployee && !hasTeam)) {
      throw ApiError.badRequest(
        ...
        'Exactly one of employeeId or teamId must be provided',
      );
    }
    ...
    if (dto.employeeId) {
      ...
      if (actor.roleCode !== 'FOUNDER') {
        if (!outranks(actor.roleCode as RoleCode, target.role.code as RoleCode)) {
          throw ApiError.forbidden(
            'ROLE_HIERARCHY_FORBIDDEN',
            'You can only configure KPI targets for roles strictly below your rank (PRD §5.1.2)',
          );
        }
      }

      // Check if period score is already frozen
      ...
      if (frozenScore?.isFrozen) {
        throw ApiError.conflict('KPI_TARGET_LOCKED', 'KPI target is locked because the period score is already frozen');
      }
    }
    ...
    await this.audit.record(this.prisma, {
      ...
      action: AuditAction.KPI_TARGET_SET,
      after: target,
    });
  ...
  async getScore(actor: AuthEmployee, employeeId: string, period: string) {
    ...
    if (!targetEmp) throw ApiError.notFound('EMPLOYEE_NOT_FOUND', 'Target employee not found');

    if (!isSelf && actor.roleCode !== 'FOUNDER') {
      if (!outranks(actor.roleCode as RoleCode, targetEmp.role.code as RoleCode)) {
        throw ApiError.forbidden(
          'ROLE_HIERARCHY_FORBIDDEN',
          'You can only view KPI scores for roles strictly below your rank (PRD §5.1.2)',
        );
      }
    }
    ...
    // Otherwise calculate live metrics
    ...
    // Follow-ups (Discipline)
    const followUps = await this.prisma.followUp.findMany({
      where: { agentId: employeeId, dueAt: { gte: startDate, lte: endDate } },
    });
    const completedFollowUps = followUps.filter((f) => f.status === 'COMPLETED').length;
    const crmDiscipline =
      followUps.length > 0 ? Number(((completedFollowUps / followUps.length) * 100).toFixed(1)) : 100;

    const actualValues: Record<string, number | null | string> = {
    ...
    if (overallScore >= 95) performanceStatus = 'Exceeds Expectations';
    else if (overallScore < 65) performanceStatus = 'Unsatisfactory';
    else if (overallScore < 80) performanceStatus = 'Needs Improvement';
  ...
  async compute(actor: AuthEmployee, dto: ComputeKpiDto) {
    if (actor.roleCode !== 'FOUNDER' && actor.roleCode !== 'MANAGER') {
    ...
    for (const emp of employees) {
      ...
      const periodScore = await this.prisma.kpiPeriodScore.upsert({
        ...
        create: {
          employeeId: emp.id,
          period,
          actualValues: computed.actualValues as any,
          targetValues: computed.targetValues as any,
          weights: computed.weights as any,
          achievementPercentages: computed.achievementPercentages as any,
    ...
    await this.audit.record(this.prisma, {
      ...
      action: AuditAction.KPI_COMPUTED,
  ...
  async addReviewEntry(actor: AuthEmployee, dto: CreateKpiReviewEntryDto) {
    if (actor.roleCode !== 'FOUNDER' && actor.roleCode !== 'MANAGER') {
    ...
    if (!scoreId) {
      throw ApiError.notFound('KPI_SCORE_NOT_FOUND', 'Target KPI period score not found');
    }

    const periodScore = await this.prisma.kpiPeriodScore.findUnique({
      where: { id: scoreId },
    });
    if (!periodScore) throw ApiError.notFound('KPI_SCORE_NOT_FOUND', 'Target KPI period score not found');

    if (periodScore.isFrozen) {
      throw ApiError.conflict('KPI_SCORE_FROZEN', 'Cannot add review entries to a frozen period score');
    }
  ...
  async freezeScore(
    ...
    if (actor.roleCode !== 'FOUNDER') {
      if (!outranks(actor.roleCode as RoleCode, targetEmp.role.code as RoleCode)) {
        throw ApiError.forbidden(
          'ROLE_HIERARCHY_FORBIDDEN',
          'You can only freeze KPI scores for roles strictly below your rank (PRD §5.1.2)',
        );
      }
    }

    // Get current live score first
    ...
    const periodScore = await this.prisma.kpiPeriodScore.upsert({
      ...
      create: {
        employeeId,
        period,
        actualValues: liveScore.actualValues as any,
        targetValues: liveScore.targetValues as any,
        weights: liveScore.weights as any,
        achievementPercentages: liveScore.achievementPercentages as any,
    ...
    await this.audit.record(this.prisma, {
      ...
      action: AuditAction.KPI_SCORE_FROZEN,
      after: periodScore,
    });
  ...
  async getTeamSummary(actor: AuthEmployee, period: string) {
