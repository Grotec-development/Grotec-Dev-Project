...
interface RMItem {
  customer: { id: string; farmerCode: string | null; fullName: string; primaryPhone: string | null };
  owner: { id: string; fullName: string } | null;
  assignedAt: string | null;
  pendingFollowUps: number;
  lastCall: { id: string; status: string; outcome: string | null } | null;
}
...
describe('relationship ownership — RM workspace, assignment, release (Month 4)', () => {
  ...
  async function salesConvertedCustomer(phone: string): Promise<string> {
  ...
  async function rmList(token: string, query = ''): Promise<RMItem[]> {
    ...
    return (res.body as { items: RMItem[] }).items;
  }
  ...
  it('founder reassigns to another RM holder (audited); manager cannot touch another portfolio', async () => {
    ...
    const res = await request(app.getHttpServer())
      ...
      .expect(201);
    expect((res.body as { owner: { id: string } }).owner.id).toBe(secondManagerId);
  ...
  it('release + unassigned list: manager claims an unassigned converted customer to themselves', async () => {
    ...
    const unassigned = (await request(app.getHttpServer())
      ...
      .set('Authorization', `Bearer ${founderToken}`)
      .expect(200)).body as { items: RMItem[] };
  ...
  it('customer notes: add + list with audit; scoped visibility; staff forbidden', async () => {
    ...
    const added = await request(app.getHttpServer())
      ...
      .expect(201);
    const noteId = (added.body as { id: string }).id;
    expect((added.body as { author: { fullName: string } }).author.fullName).toBe('Manager One');
    expect(await prisma.auditEvent.count({ where: { entityType: 'CUSTOMER_NOTE', action: 'customer.note_added' } })).toBe(1);

    const notes = (await request(app.getHttpServer()).get(`/api/v1/customers/${customerId}/notes`).set('Authorization', `Bearer ${managerToken}`).expect(200)).body as Array<{ id: string; body: string }>;
