...
let KpiService = class KpiService {
    ...
    async getTargets(actor, query) {
        ...
        if (actor.roleCode === 'FOUNDER') {
            ...
            if (query.employeeId) {
                if (query.employeeId !== actor.id) {
                    ...
                    if (!target || !outranks(actor.roleCode, target.role.code)) {
                        throw ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'You can only view KPI targets for roles strictly below your rank (PRD §5.1.2)');
                    }
                }
                conditions.push({ employeeId: query.employeeId });
            }
            ...
            if (query.employeeId && query.employeeId !== actor.id) {
                throw ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'You can only view your own KPI targets (PRD §5.1.2)');
            }
    ...
    async upsertTarget(actor, dto) {
        ...
        if ((hasEmployee && hasTeam) || (!hasEmployee && !hasTeam)) {
            throw ApiError.badRequest('EXACTLY_ONE_SCOPE_REQUIRED', 'Exactly one of employeeId or teamId must be provided');
        }
        if (dto.employeeId) {
            const target = await this.prisma.employee.findUnique({
