...
let PayrollService = PayrollService_1 = class PayrollService {
    ...
    async getRun(actor, id) {
        ...
        if (!run)
            throw ApiError.notFound('PAYROLL_RUN_NOT_FOUND', 'Payroll run not found');
        return run;
    }
    ...
    async generate(actor, dto) {
        ...
        if (existingRun &&
            ...
            throw ApiError.conflict('PAYROLL_RUN_LOCKED', `Payroll run for ${dto.month} is ${existingRun.status} and cannot be recalculated`);
        }
    ...
    async approve(actor, id, dto) {
        ...
        if (hasCriticalFlags && dto?.acknowledgeFlags !== true) {
            throw ApiError.badRequest('UNACKNOWLEDGED_FLAGS', 'Calculation flags (NEGATIVE_NET_PAY or NO_SALARY_REVISION) must be acknowledged before approval');
        }
    ...
    async publish(actor, id) {
        ...
        // Transaction 1 — financial state only: publish the run, write audit.
        // Side effects (advance recovery, notifications) are now emitted as domain events
        // to the outbox and processed asynchronously by OutboxWorker.
        const published = await this.prisma.$transaction(async (tx) => {
            ...
            // Emit payroll.published event to the outbox — co-committed with the financial state.
            // OutboxWorker processes it asynchronously and fans out to advance-recovery ledger
            // updates and per-employee AppNotifications (at-least-once delivery).
            await this.domainEvents.emit(tx, {
                eventType: DOMAIN_EVENTS.PAYROLL_PUBLISHED,
    ...
    async getPayslips(actor, employeeId) {
        ...
        if (actor.roleCode === 'FOUNDER') {
            ...
            if (employeeId) {
                if (employeeId !== actor.id) {
                    ...
                    if (!target || !outranks(actor.roleCode, target.role.code)) {
                        throw ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'You can only view payslips for roles strictly below your rank (PRD §5.1.2)');
                    }
                }
                conditions.push({ employeeId });
            }
            ...
            if (employeeId && employeeId !== actor.id) {
                throw ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'You can only view your own payslips (PRD §5.1.2)');
            }
    ...
    async getPayslipDetail(actor, id) {
        ...
        if (!item)
            throw ApiError.notFound('PAYSLIP_NOT_FOUND', 'Payslip not found');
