...
let AttendanceService = class AttendanceService {
    ...
    async list(actor, pagination, filters) {
        ...
        if (actor.roleCode === 'FOUNDER') {
            ...
            if (filters.employeeId) {
                if (filters.employeeId !== actor.id) {
                    ...
                    if (!target || !(0, shared_1.outranks)(actor.roleCode, target.role.code)) {
                        throw api_error_1.ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'You can only view attendance for roles strictly below your rank (PRD §5.1.2)');
                    }
                }
                conditions.push({ employeeId: filters.employeeId });
            }
            ...
            if (filters.employeeId && filters.employeeId !== actor.id) {
                ...
                if (!assigned) {
                    throw api_error_1.ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'You can only view attendance for yourself or assigned employees');
                }
                conditions.push({ employeeId: filters.employeeId });
            }
            else {
            ...
            if (filters.employeeId && filters.employeeId !== actor.id) {
                throw api_error_1.ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'You can only view your own attendance records (PRD §5.1.2)');
            }
    ...
    async markMy(actor, dto) {
        if (actor.roleCode === 'FOUNDER') {
            throw api_error_1.ApiError.badRequest('FOUNDER_ATTENDANCE_NOT_APPLICABLE', 'Founder / CEO is the business owner and is not tracked for attendance');
        }
        ...
        if (actor.roleCode === 'FOUNDER') {
            ...
            if (query.employeeId) {
                if (query.employeeId !== actor.id) {
                    ...
                    if (!target || !(0, shared_1.outranks)(actor.roleCode, target.role.code)) {
                        throw api_error_1.ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'You can only view attendance summary for roles strictly below your rank (PRD §5.1.2)');
                    }
                }
                conditions.push({ employeeId: query.employeeId });
            }
            ...
            if (query.employeeId && query.employeeId !== actor.id) {
                throw api_error_1.ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'You can only view your own attendance summary (PRD §5.1.2)');
            }
            conditions.push({ employeeId: actor.id });
        }
    ...
    async mark(actor, dto) {
        ...
        if (actor.roleCode === 'STAFF' && !isSelf) {
            throw api_error_1.ApiError.forbidden('ATTENDANCE_SELF_ONLY', 'Staff may only mark their own attendance');
        }
        ...
        if (!isSelf) {
            if (!(0, shared_1.outranks)(actor.roleCode, employee.role.code)) {
                throw api_error_1.ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'You can only mark attendance for roles strictly below your rank (PRD §5.1.2)');
            }
        }
        ...
        if (existing) {
            throw api_error_1.ApiError.conflict('ATTENDANCE_ALREADY_EXISTS', 'Attendance record already exists for this date');
        }
        const canApprove = actor.permissions.includes(shared_1.PERMISSIONS.attendanceApprove) && !isSelf;
        const initialApprovalStatus = canApprove
            ? shared_1.ApprovalStatus.APPROVED
            : shared_1.ApprovalStatus.PENDING;
        ...
        const record = await this.prisma.$transaction(async (tx) => {
            ...
            await this.audit.record(tx, {
                ...
                action: shared_1.AuditAction.ATTENDANCE_MARKED,
                after: created,
            });
            return created;
        });
    ...
    async bulkMark(actor, dto) {
        if (actor.roleCode === 'STAFF' || actor.roleCode === 'AGENT' || actor.roleCode === 'DELIVERY') {
            throw api_error_1.ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'Staff and agents cannot perform bulk attendance operations');
        }
        const canApprove = actor.permissions.includes(shared_1.PERMISSIONS.attendanceApprove);
        const date = parseDateOnly(dto.date);
        if (actor.roleCode !== 'FOUNDER') {
            ...
            for (const emp of employees) {
                if (!(0, shared_1.outranks)(actor.roleCode, emp.role.code)) {
                    throw api_error_1.ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', `Cannot mark attendance for employee ${emp.fullName} at or above your rank (PRD §5.1.2)`);
                }
            }
        }
        const initialApprovalStatus = canApprove
    ...
    async correct(actor, id, dto) {
        if (!dto.reason || dto.reason.trim().length < 10) {
            throw api_error_1.ApiError.badRequest('REASON_REQUIRED', 'Correction reason is required (minimum 10 characters)');
        }
        const existing = await this.prisma.attendanceRecord.findUnique({
            where: { id },
        ...
        if (!existing)
            throw api_error_1.ApiError.notFound('ATTENDANCE_NOT_FOUND', 'Attendance record not found');
        if (actor.roleCode !== 'FOUNDER') {
            if (!(0, shared_1.outranks)(actor.roleCode, existing.employee.role.code)) {
                throw api_error_1.ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'You can only correct attendance for roles strictly below your rank (PRD §5.1.2)');
            }
        }
        const resetToPending = existing.approvalStatus === shared_1.ApprovalStatus.APPROVED;
        const newApprovalStatus = resetToPending ? shared_1.ApprovalStatus.PENDING : existing.approvalStatus;
        ...
        const updated = await this.prisma.$transaction(async (tx) => {
            ...
            await this.audit.record(tx, {
                ...
                action: shared_1.AuditAction.ATTENDANCE_CORRECTED,
                before: existing,
                after: rec,
            });
            return rec;
    ...
    async approve(actor, id) {
        ...
        if (!existing)
            throw api_error_1.ApiError.notFound('ATTENDANCE_NOT_FOUND', 'Attendance record not found');
        if (existing.approvalStatus !== shared_1.ApprovalStatus.PENDING) {
            throw api_error_1.ApiError.conflict('ATTENDANCE_ALREADY_DECIDED', 'Attendance has already been decided');
        }
        if (actor.id === existing.employeeId) {
            throw api_error_1.ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'You cannot approve your own attendance');
        }
        if (actor.roleCode !== 'FOUNDER') {
            if (!(0, shared_1.outranks)(actor.roleCode, existing.employee.role.code)) {
                throw api_error_1.ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'You can only approve attendance for roles strictly below your rank (PRD §5.1.2)');
            }
        }
        const updated = await this.prisma.$transaction(async (tx) => {
            const rec = await tx.attendanceRecord.update({
            ...
            await tx.attendanceApprovalHistory.create({
                data: {
                    ...
                    action: 'APPROVED',
                    fromStatus: existing.approvalStatus,
                    toStatus: shared_1.ApprovalStatus.APPROVED,
            ...
            await tx.appNotification.create({
                data: {
                    ...
                    type: shared_1.NotificationType.ATTENDANCE_STATUS,
            ...
            await this.audit.record(tx, {
                ...
                action: shared_1.AuditAction.ATTENDANCE_APPROVED,
                before: existing,
                after: rec,
            });
            return rec;
    ...
    async reject(actor, id, dto) {
        ...
        if (actor.id === existing.employeeId) {
            throw api_error_1.ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'You cannot reject your own attendance');
        }
        if (actor.roleCode !== 'FOUNDER') {
            if (!(0, shared_1.outranks)(actor.roleCode, existing.employee.role.code)) {
                throw api_error_1.ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'You can only reject attendance for roles strictly below your rank (PRD §5.1.2)');
            }
        }
        const updated = await this.prisma.$transaction(async (tx) => {
            const rec = await tx.attendanceRecord.update({
                where: { id },
                data: {
                    approvalStatus: shared_1.ApprovalStatus.REJECTED,
                    approverId: actor.id,
                    approvedAt: new Date(),
            ...
            await tx.attendanceApprovalHistory.create({
                data: {
                    ...
                    action: 'REJECTED',
                    fromStatus: existing.approvalStatus,
                    toStatus: shared_1.ApprovalStatus.REJECTED,
                    reason: dto.reason,
                },
            });
            await tx.appNotification.create({
            ...
            await this.audit.record(tx, {
                ...
                action: shared_1.AuditAction.ATTENDANCE_REJECTED,
                before: existing,
                after: rec,
            });
            return rec;
    ...
    async handleEsslWebhook(secret, dto) {
        ...
        if (!secret || secret !== expectedSecret) {
            throw api_error_1.ApiError.unauthorized('INVALID_WEBHOOK_SECRET', 'Invalid ESSL webhook secret');
        }
        ...
        if (!device) {
            throw api_error_1.ApiError.badRequest('UNMAPPED_DEVICE', 'Device is not registered in the system');
        }
            ...
            if (employeeId) {
                ...
                await this.prisma.attendanceRecord.upsert({
                    ...
                    create: {
                        ...
                        source: shared_1.AttendanceSource.ESSL,
                        punchIn: firstPunch,
                        punchOut: lastPunch,
    ...
    async syncEssl(actor, dto) {
        if (actor.roleCode === 'AGENT') {
            throw api_error_1.ApiError.forbidden('ROLE_HIERARCHY_FORBIDDEN', 'Agents cannot trigger biometric hardware sync');
        }
        ...
        if (devices.length === 0) {
            throw api_error_1.ApiError.badRequest('NO_DEVICES', 'No active ESSL devices configured');
        }
        ...
        const results = await this.prisma.$transaction(async (tx) => {
            ...
            await this.audit.record(tx, {
                ...
                action: shared_1.AuditAction.ATTENDANCE_SYNCED_ESSL,
    ...
    async createDevice(dto) {
        ...
        if (existing)
            throw api_error_1.ApiError.conflict('DEVICE_EXISTS', 'Device code already registered');
    ...
    async createMapping(dto) {
        ...
        if (existing)
            throw api_error_1.ApiError.conflict('MAPPING_EXISTS', 'Biometric PIN already mapped on this device');
