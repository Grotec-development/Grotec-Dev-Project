...
let CustomersService = class CustomersService {
    ...
    async list(actor, pagination, filters) {
        ...
        if (filters.phone) {
            const e164 = (0, shared_1.normalizePhoneToE164)(filters.phone);
            if (e164)
                conditions.push({ phones: { some: { ...LIVE_PHONE, phoneE164: e164 } } });
            else
                return (0, pagination_1.toPage)([], 0, pagination);
        }
        if (filters.status)
        ...
        return (0, pagination_1.toPage)(rows.map((customer) => ({
            id: customer.id,
    ...
    async lookupByPhone(rawPhone) {
        const e164 = (0, shared_1.normalizePhoneToE164)(rawPhone);
        if (!e164)
            throw api_error_1.ApiError.badRequest('INVALID_PHONE', 'Phone number could not be normalized');
        const match = await this.prisma.customerPhone.findFirst({
            where: { ...LIVE_PHONE, phoneE164: e164, customer: { deletedAt: null } },
            include: { customer: true },
        });
        if (!match) {
            throw api_error_1.ApiError.notFound('CUSTOMER_NOT_FOUND', 'No customer found for this number');
        }
        return {
    ...
    async create(actor, dto) {
        ...
        if (phoneInputs.filter((p) => p.isPrimary === true).length > 1) {
            throw api_error_1.ApiError.badRequest('SINGLE_PRIMARY_PHONE', 'Only one phone can be primary');
        }
        const primaryIndex = desiredPrimaryIndex === -1 ? 0 : desiredPrimaryIndex;
        const seen = new Set();
        for (const e164 of normalized) {
            if (seen.has(e164))
                throw api_error_1.ApiError.badRequest('DUPLICATE_PHONE_IN_REQUEST', `Phone ${e164} listed more than once`);
            seen.add(e164);
        }
        ...
        const created = await this.prisma.$transaction(async (tx) => {
            ...
            const orphanedCalls = await tx.call.findMany({
                where: { customerId: null, phoneNumber: { in: [...seen] }, status: { in: [...shared_1.ACTIVE_CALL_STATUSES] } },
            });
    ...
    async setActive(actor, id, active) {
        ...
        await this.prisma.$transaction(async (tx) => {
            await tx.customer.update({
                where: { id },
                data: { status: active ? client_1.CustomerStatus.ACTIVE : client_1.CustomerStatus.INACTIVE },
            });
            await this.audit.record(tx, {
                actorId: actor.id,
                entityType: 'CUSTOMER',
                ...
                after: { status: active ? client_1.CustomerStatus.ACTIVE : client_1.CustomerStatus.INACTIVE },
            });
        });
    }
    async addPhone(actor, customerId, input) {
        ...
        if (dup) {
            if (dup.customerId === customerId) {
                throw api_error_1.ApiError.conflict('PHONE_ALREADY_EXISTS', 'This number is already on the customer profile', {
                    phoneId: dup.id,
                });
            }
    ...
    async updatePhone(actor, customerId, phoneId, input) {
        ...
        if (input.isPrimary === true) {
            ...
            if (input.isPrimary === false && phone.isPrimary) {
                ...
                if (live <= 1) {
                    throw api_error_1.ApiError.badRequest('LAST_PRIMARY_PHONE', 'A customer must keep one primary phone');
                }
            }
            await this.prisma.$transaction(async (tx) => {
    ...
    async removePhone(actor, customerId, phoneId) {
        ...
        await this.prisma.$transaction(async (tx) => {
            ...
            if (liveCount <= 1) {
                throw api_error_1.ApiError.badRequest('LAST_PHONE_REQUIRED', 'A customer must keep at least one phone number');
            }
    ...
    async updateLocation(actor, customerId, locationId, input) {
        ...
        if (!location)
            throw api_error_1.ApiError.notFound('LOCATION_NOT_FOUND', 'Location not found');
        await this.prisma.$transaction(async (tx) => {
            if (input.isPrimary === true) {
                await tx.customerLocation.updateMany({ where: { customerId, deletedAt: null }, data: { isPrimary: false } });
            }
    ...
    async addCrop(actor, customerId, input) {
        ...
        if (existing)
            throw api_error_1.ApiError.conflict('CUSTOMER_CROP_EXISTS', 'This crop is already recorded for the customer');
        await this.prisma.$transaction(async (tx) => {
    ...
    async updateCrop(actor, customerId, customerCropId, input) {
        ...
        if (!row)
            throw api_error_1.ApiError.notFound('CUSTOMER_CROP_NOT_FOUND', 'Crop entry not found');
        await this.prisma.$transaction(async (tx) => {
    ...
    async livePhoneOf(customerId, phoneId) {
        ...
        if (!phone)
            throw api_error_1.ApiError.notFound('PHONE_NOT_FOUND', 'Phone not found on this customer');
        return phone;
    }
    ...
    normalizeOne(raw) {
        const e164 = (0, shared_1.normalizePhoneToE164)(raw);
        if (!e164) {
            throw api_error_1.ApiError.badRequest('INVALID_PHONE', `Phone number could not be normalized: "${raw}"`, { value: raw });
        }
        return e164;
    }
    phoneConflict(dup) {
        return api_error_1.ApiError.conflict('CUSTOMER_PHONE_EXISTS', 'A customer with this phone number already exists', {
            matchedCustomer: {
                id: dup.customer.id,
    ...
    validateLocations(inputs) {
        ...
        if (primaries > 1)
            throw api_error_1.ApiError.badRequest('SINGLE_PRIMARY_LOCATION', 'Only one location can be primary');
        return inputs;
    }
    async validateCrops(inputs, actor) {
        ...
        if (found.length !== ids.length) {
            throw api_error_1.ApiError.badRequest('CROP_NOT_FOUND', 'One or more crops do not exist or are inactive');
        }
        return inputs.map((c) => ({
            cropId: c.cropId,
            acreage: c.acreage,
    ...
    async requireActiveCrop(cropId) {
        ...
        if (!crop)
            throw api_error_1.ApiError.badRequest('CROP_NOT_FOUND', 'Crop does not exist or is inactive');
    }
};
