...
describe('core integrity — DB constraints and transaction correctness', () => {
  ...
  it(
    'DB constraint: exactly one active relationship owner per customer — ' +
      'inserting a second active row for the same customer fails at the DB level',
    async () => {
      ...
      try {
        ...
        constraintError = err as Error;
      }
      expect(constraintError).not.toBeNull();
      // PostgreSQL unique constraint violation code
      expect(
        constraintError!.message.includes('relationship_ownership_current_customer_idx') ||
  ...
  it('DB constraint: exactly one active lead owner per lead — second active insert fails', async () => {
    ...
    expect(constraintError).not.toBeNull();
    expect(
      constraintError!.message.includes('lead_ownership_current_lead_idx') ||
        constraintError!.message.includes('duplicate key') ||
        constraintError!.message.includes('unique constraint'),
    ).toBe(true);
  });
  ...
  it('publish atomically writes payroll_run status + audit_event; a failure rolls back both', async () => {
    ...
    const genRes = await request(app.getHttpServer())
      ...
      .send({ month })
      .expect(201);
    const runId = (genRes.body as { id: string }).id;
  ...
  it(
    'concurrent relationship ownership: two transactions racing to assign the same customer — ' +
      'only one succeeds, the other fails at the DB partial unique constraint',
    async () => {
      const managerRole = await prisma.role.findUniqueOrThrow({ where: { code: 'MANAGER' } });
      const managers = await prisma.employee.findMany({ where: { roleId: managerRole.id, status: 'ACTIVE' } });
      expect(managers.length).toBeGreaterThanOrEqual(2);
      ...
      const rejectionReason = rejection.reason as { code?: string; meta?: { target?: string[] } };
  ...
  it('concurrent lead ownership: two transactions racing to assign the same lead — DB constraint catches it', async () => {
    ...
    const rejection = rejected[0] as PromiseRejectedResult;
    const rejectionReason = rejection.reason as { code?: string };
    expect(rejectionReason.code).toBe('P2002');
