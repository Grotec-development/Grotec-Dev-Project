...
describe('calls — agent calling workspace (Month 2)', () => {
    ...
    async function placeCall(phone, token = agentToken, extra = {}) {
    ...
    async function getCall(id, token = agentToken) {
    ...
    it('rejects a second active call for the same agent', async () => {
        ...
        expect(res.body.error.details.callId).toBe(call.id);
    });
    ...
    it('queue: shows the agent their open owned leads with customer context and last call', async () => {
        ...
        const queue = (await request(app.getHttpServer()).get('/api/v1/calls/queue').set('Authorization', `Bearer ${agentToken}`).expect(200)).body;
        ...
        await sleep(40);
        await request(app.getHttpServer()).post(`/api/v1/calls/${call.id}/end`).set('Authorization', `Bearer ${agentToken}`).expect(201);
        const queue2 = (await request(app.getHttpServer()).get('/api/v1/calls/queue').set('Authorization', `Bearer ${agentToken}`).expect(200)).body;
    ...
    it('queue: agents only see their own leads; managers can filter by ownerId', async () => {
        ...
        const leadBId = leadB.body.id;
        const agentQueue = (await request(app.getHttpServer()).get('/api/v1/calls/queue').set('Authorization', `Bearer ${agentToken}`).expect(200)).body;
        expect(agentQueue.some((q) => q.leadId === leadBId)).toBe(false);
        const founderQueue = (await request(app.getHttpServer()).get('/api/v1/calls/queue').set('Authorization', `Bearer ${founderToken}`).expect(200)).body;
    ...
    it('notes: agent adds a note visible on the call record', async () => {
        ...
        const note = await request(app.getHttpServer())
            ...
            .expect(201);
        expect(note.body.body).toBe('Farmer asked about rice seed availability');
        const detail = await getCall(call.id);
    ...
    it('call history is exposed under the customer', async () => {
        ...
        await sleep(30);
        await request(app.getHttpServer()).post(`/api/v1/calls/${call.id}/end`).set('Authorization', `Bearer ${agentToken}`).expect(201);
        const history = (await request(app.getHttpServer()).get(`/api/v1/customers/${customerId}/calls`).set('Authorization', `Bearer ${agentToken}`).expect(200)).body;
    ...
    it('context endpoint returns the call, customer profile, and history', async () => {
        ...
        const ctx = (await request(app.getHttpServer()).get(`/api/v1/calls/${call.id}/context`).set('Authorization', `Bearer ${agentToken}`).expect(200)).body;
    ...
    it('authorization: an agent cannot link a call to a lead they do not own', async () => {
        // A second agent owns a lead the first agent must not be able to link a call to.
        const agentRole = await prisma.role.findUniqueOrThrow({ where: { code: 'AGENT' } });
        const email = uniqueEmail('agent2');
        await request(app.getHttpServer())
            .post('/api/v1/employees')
            ...
            .expect(201);
        const agent2Token = await loginToken(app, { email, password: 'TempPass1', roleCode: 'AGENT' });
        const otherCustomerId = await createCustomerAs(app, agent2Token, 'Other Agent Lead Farmer', '9876546015');
        const otherLead = await request(app.getHttpServer())
            .post('/api/v1/leads')
            .set('Authorization', `Bearer ${agent2Token}`)
            .send({ customerId: otherCustomerId, source: 'TEST' })
            .expect(201);
        const otherLeadId = otherLead.body.id;
        const res = await request(app.getHttpServer())
            .post('/api/v1/calls')
            .set('Authorization', `Bearer ${agentToken}`)
            .send({ phoneNumber: '9876546016', leadId: otherLeadId })
            .expect(403);
        expect(res.body.error.code).toBe('LEAD_LINK_FORBIDDEN');
        // A manager (permitted cross-agent linking) can place a call linked to the same lead.
        await request(app.getHttpServer())
            .post('/api/v1/calls')
            .set('Authorization', `Bearer ${managerToken}`)
            .send({ phoneNumber: '9876546017', leadId: otherLeadId })
            .expect(201);
    });
});
