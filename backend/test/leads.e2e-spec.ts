...
describe('leads + ownership', () => {
  ...
  async function makeCustomerAndLead(name: string, phone: string, token = agentToken): Promise<{ customerId: string; leadId: string }> {
    ...
    const lead = await request(app.getHttpServer())
      ...
      .expect(201);
    return { customerId, leadId: (lead.body as { id: string }).id };
  }
  ...
  it('creates a lead auto-assigned to the creating agent', async () => {
    ...
    const body = detail.body as {
      customer: { id: string; primaryPhone: string };
      currentOwner: { id: string };
      ownershipHistory: Array<{ isCurrent: boolean }>;
    };
  ...
  it('reassigns a lead (manager) and closes the previous ownership window', async () => {
    ...
    const res = await request(app.getHttpServer())
      ...
      .expect(201);
    const body = res.body as { currentOwner: { id: string }; ownershipHistory: Array<{ isCurrent: boolean }> };
  ...
  it('cannot assign a lead to an inactive employee', async () => {
    ...
    const created = await request(app.getHttpServer())
      ...
      .expect(201);
    const inactiveId = (created.body as { employee: { id: string } }).employee.id;
  ...
  it('updates lead notes/source', async () => {
    ...
    const res = await request(app.getHttpServer())
      ...
      .expect(200);
    const body = res.body as { notes: string; source: string };
