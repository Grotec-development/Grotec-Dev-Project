...
describe('call outcomes — exactly three, separate next action (Month 3)', () => {
    ...
    async function finishedCall(phone = '9876547001') {
    ...
    async function recordOutcome(callId, body, token = agentToken) {
    ...
    it('rejects recording an outcome while the call is still active', async () => {
        ...
        const res = await request(app.getHttpServer())
            .post(`/api/v1/calls/${placed.body.id}/outcome`)
            .set('Authorization', `Bearer ${agentToken}`)
    ...
    it('Interested -> Callback requires date + time + reason and creates a follow-up record', async () => {
        ...
        const followUp = await prisma.followUp.findUniqueOrThrow({ where: { id: ok.body.followUp?.id } });
    ...
    it('Not Answered stores the outcome + attempt history only (no retry, no automatic follow-up)', async () => {
        ...
        const placed = await request(app.getHttpServer())
            ...
            .expect(201);
        const noAnswerCall = placed.body.id;
        await sleep(60);
    ...
    it('follow-ups: agents see their own callbacks; manager sees all; staff is forbidden', async () => {
        ...
        const agentList = (await request(app.getHttpServer()).get('/api/v1/follow-ups').set('Authorization', `Bearer ${agentToken}`).expect(200)).body;
        expect(agentList.length).toBeGreaterThanOrEqual(1);
        expect(agentList.every((f) => f.status === 'PENDING')).toBe(true);
        const managerList = (await request(app.getHttpServer()).get('/api/v1/follow-ups?status=PENDING').set('Authorization', `Bearer ${managerToken}`).expect(200)).body;
